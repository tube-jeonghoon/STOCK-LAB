...
class EBest:
    ...
    def __init__(self, mode=None):
        ...
        query_cnt는 10분당 200개의 TR수행을 관리하기 위한 리스트
        xa_sessiono_client는 XASession 객체
    ...
    def _execute_query(
        self, res, in_block_name, out_block_name, *out_fields, **set_fields
    ):
        """
        ...
        :return result:list 결과를 list 에 담아 반환
        """
        time.sleep(1)
        ...
        while len(self.query_cnt) >= EBest.QUERY_LIMIT_10MIN:
            time.sleep(1)
            print(
                "waiting for execute query... current query cnt:", len(self.query_cnt)
            )
            self.query_cnt = list(
                filter(
                    lambda x: (datetime.today() - x).total_seconds()
                    < EBest.LIMIT_SECONDS,
                    self.query_cnt,
                )
            )
